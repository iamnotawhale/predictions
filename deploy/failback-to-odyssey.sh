...
# обновить env на Odyssey под :8443 (на случай если кто-то менял)
ssh -o BatchMode=yes -o ConnectTimeout=12 -p "$ODYSSEY_SSH_PORT" "${ODYSSEY_SSH_USER}@${IP}" \
  "grep -q '^HTTPS_PORT=' /home/nikita/predictions/deploy/predicts.env && \
   ...
   sed -i \"s|^BOT_WEBAPP_URL=.*|BOT_WEBAPP_URL=https://${DOMAIN}:${ODYSSEY_HTTPS_PORT}/miniapp/|\" /home/nikita/predictions/deploy/predicts.env || true
...
log "DONE. Mini App: https://${DOMAIN}:${ODYSSEY_HTTPS_PORT}/miniapp/"
