...
- Файлы `static/miniapp/js/app.js`, `index.html`, `css/app.css`:
  ...
  - в live-модалке над лентой событий добавлено постоянное мини-поле (горизонтальное, с разметкой и вертикальными полосами травы) в пропорции профессионального поля `105:68`; разметка отрисовывается встроенным `SVG` (стабильнее в Telegram WebView), геометрия приведена к значениям IFAB (штрафная `16.5m`, вратарская `5.5m`, точка пенальти `11m`, радиус центра/дуги `9.15m`, угловые четверти окружности `1m`), включая полуокружности штрафной; для события с координатами ESPN (`fieldPositionX/Y`, опционально `fieldPosition2X/Y`, `goalPositionY`, `team.displayName`, `shortText`) показывается маркер цвета команды (`homeColor`/`awayColor` из `team_colors.json` через `live-details`) с бейджем `мин' фамилия · ТИП` (напр. `12' Ndiaye · В СТВОР`, `42' Dewsbury-Hall · ГОЛ`; без игрока — только тип); для ударов/голов — линия `fieldStart` → `fieldEnd` (ESPN): гол — сплошная к `goalPositionY` на линии ворот; в створ — пунктир туда же; мимо/штанга — пунктир к `fieldPosition2`; блок — сплошная к `fieldPosition2` (без точки у ворот; `goalPositionY` только GoalMouth); тап по событию в ленте переключает маркер (по умолчанию — последнее с coords); маркер скрывается при открытой таблице статистики (тап по полю) и на перерыве (`HT` / «First Half ends» без признаков 2-го тайма); превью: `docs/live-pitch-preview.svg`.
