...
- **Не пушить и не деплоить автоматически** после каждого изменения: сначала уточни в чате у Никиты. Часто нужно добрать ещё правки и задеплоить всё одним скопом (`git push` / Actions / `todaypub` — только по явной просьбе).
```
## Кратко о проекте
...
- `src/main/java/zhigalin/predictions/util/TelegramMarkdownV2` — escape caption для Unirest `sendPhoto` / `sendAnimation`.
...
- `DataInitService`: адаптивный sync (30с при live/ближайших матчах, иначе 120с); early-exit через `hasPostponedMatches()` (SQL `EXISTS`) + `findOnlineMatches` (окно дат без `pst`); scoreboard — только через `EspnScoreboardClient`; при **любом** изменении суммы голов в live шлёт `sendLiveScoreUpdate`: при GIF — **одно** сообщение-анимация (`sendAnimation` первый гол, далее in-place `editMessageMedia`; caption = счёт/авторы); если GIF выкл/упал — edit текста. На **FT** (`becameFinished` / `sendFullTime`) — `removeLiveScoreNoise` удаляет live GIF/текст из чата. Авторы голов **без ассистов** (`listGoalScorers` / `Goal!` + `Own Goal by …` → `Name (авт.)`, в caption счётом после каждого гола: `0:1 Salah 23', 1:1 Foden 61'`; строк больше total → первые N); ↑total → «ГОЛ!», ↓total → «ОТМЕНЁН»; флаг `notification.goal-gif.enabled` / `NOTIFICATION_GOAL_GIF_ENABLED`); при входе в live и catch-up на FT — `BettingRecommendationService.freezeAtKickoffIfNeeded`; при переходе матча в `post/ft` — сразу `predictionService.updateByMatch` (очки не зависят только от LISTEN→`sendFullTime`), плюс очистка кэша составов через `ApiClient.evictLineups(publicId)`.
...
- **Скользящий пол очков:** только **общий зачёт / график** — `running = max(0, running + points)` в порядке `coalesce(finished_at, local_date_time)` (`FlooredPointsService` / `PointEventDao`). **Очки за тур** (leaderboard `?weekId=`, Telegram `/total N`, weekly card, live provisional тура) — **сырая сумма** `predict.points` (может быть отрицательной). Сырые `predict.points` не затираются.
- **Персональный бонус тура:** `user_week_bonus_match` — у каждого игрока свой случайный матч **только текущего** тура; выдача **один раз** при смене current week (`updateCurrent` → `onCurrentWeekRollover` → `assignForNewCurrentWeek`); строки на будущие туры удаляются; прошлые назначения остаются для скоринга/бейджа в архиве. UI не создаёт бонус при открытии чужого тура. С **5 тура** (`MIN_WEEK_ID`).
...
- **`HtmlImageRenderer`**: все Telegram-карточки 1080×1080 HTML→PNG (Playwright/Chromium): сегодняшние матчи, напоминание без прогноза, FT EPL/кубки, weekly results, «твой прогноз», график очков (JFreeChart внутри HTML-shell). CSS: `src/main/resources/notification-cards/base.css`. Класс `card-narrow` (поля 90px, панели `min(900px,100%)`, `align-items: center`) — today/FT/weekly/predict/chart; **reminder без сужения**. Chrome: `notification.cards.chromeExecutable` / `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` / `/usr/bin/google-chrome`. Семафор на 1 рендер. Превью: `./mvnw -Dtest=HtmlCardPreviewTest test` → `target/preview-cards/`.
...
| GET | `/profile/stats` | Статистика прогнозов пользователя (exact%/исход; `teams`: ср. очки по всем клубам АПЛ; привычки П1/X/П2 + тотал прогноз/факт; лучший/слабый тур; без сравнений с другими) |
...
| GET | `/logos/r/{hash}.png` | Прокси логотипа по URL (кубки) |
...
1. Автоматически в `DataInitService` после завершения всех матчей тура (`weekService.updateCurrent()` → `onCurrentWeekRollover`: `assignForNewCurrentWeek` + `refreshForWeek` следующего тура).
...
- **Профиль** (`screen-my`): hero как у клуба + статистика сезона (`GET /profile/stats`, `UserPredictionStatsService`): блок **По командам** — все клубы АПЛ в 2 колонки со средними очками (без «Заходит/Мимо»); **Привычки и тотал** — тип ставки, ср. очки П1/X/П2, тотал прогноз/факт; кнопка **Мои прогнозы** → сетка туров / кубки / разбор как раньше.
...
- **Live goal GIF:** `GoalEventGifRenderer` (AWT, 400×400, ~3.5с) — фон `notification-cards/goal-anim-bg.png`, логотипы через `TeamLogoCacheService` + ESPN fallback; `GifSequenceWriter` (полные кадры) + **`gifsicle -O0`**. Первый гол — `sendAnimation`, следующие — `editMessageMedia` (`bot.urlEditMessageMedia`). На FT — `removeLiveScoreNoise` (delete live message). Превью: `./mvnw -Dtest=GoalEventGifPreviewTest test`. Выкл.: `NOTIFICATION_GOAL_GIF_ENABLED=false`.
...
- **Логотипы клубов АПЛ:** miniapp отдаёт same-origin `/api/miniapp/logos/{teamId}.png` (`TeamLogoCacheService`: API-Football media → ESPN fallback, память + диск, warmup на старте; `Cache-Control: public, max-age=30d, immutable`). Кубки без EPL `team_id` — `/api/miniapp/logos/r/{hash}.png`. HTML-карточки по-прежнему встраивают data URI с `media.api-sports.io` (Playwright). Navigate: `domcontentloaded`.
