...
- Файлы `static/miniapp/js/app.js`, `index.html`, `css/app.css`:
  ...
  - текст live-событий в miniapp переводится на русский шаблонно (без перевода имен игроков/команд): для типовых ESPN-фраз (`Foul by ...`, `... wins a free kick ...`, `Offside...`, `Corner...`, `Substitution...`, `Attempt ...`, `Goal! ...` и т.д.) применяется RU-формат с fallback на оригинал.
  - в live-модалке над лентой событий добавлено постоянное мини-поле (горизонтальное, с разметкой и вертикальными полосами травы) в пропорции профессионального поля `105:68`; разметка отрисовывается встроенным `SVG` (стабильнее в Telegram WebView), геометрия приведена к значениям IFAB (штрафная `16.5m`, вратарская `5.5m`, точка пенальти `11m`, радиус центра/дуги `9.15m`, угловые четверти окружности `1m`), включая полуокружности штрафной; для последнего события с координатами из ESPN (`play.fieldPositionX/Y`) показывается белый маркер с бейджем `минута + тип` (`67' GOAL`), при этом позиция бейджа динамическая по половинам поля (верх/низ, лево/право) и с отступом от точки; маркер скрывается при открытой таблице статистики (тап по полю) и на перерыве (`HT` / «First Half ends» без признаков 2-го тайма — событие `Second Half begins` или минута `>= 46'` в статусе/ленте).
