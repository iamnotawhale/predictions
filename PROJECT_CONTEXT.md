...
- Файлы `static/miniapp/js/app.js`, `index.html`, `css/app.css`:
  ...
  - в live-модалке над лентой событий добавлено постоянное мини-поле (горизонтальное, с разметкой и вертикальными полосами травы) в пропорции профессионального поля `105:68`; геометрия разметки приведена к значениям IFAB (штрафная `16.5m`, вратарская `5.5m`, точка пенальти `11m`, радиус центра/дуги `9.15m`), включая полуокружности штрафной; для последнего события с координатами из ESPN (`play.fieldPositionX/Y`) показывается белый маркер с бейджем `минута + тип` (`67' GOAL`), при этом позиция бейджа динамическая по половинам поля (верх/низ, лево/право) и с отступом от точки.
