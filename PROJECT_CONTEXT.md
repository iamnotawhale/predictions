...
- `DataInitService`: адаптивный sync (30с при live/ближайших матчах, иначе 120с); early-exit через `hasPostponedMatches()` (SQL `EXISTS`) + `findOnlineMatches` (окно дат без `pst`); scoreboard — только через `EspnScoreboardClient`; при **любом** изменении суммы голов в live/post шлёт `sendLiveScoreUpdate` в Telegram-чат (edit того же `live_score_message_id`) со списком авторов голов **без ассистов** (`ApiClient.listGoalScorers` / commentary `Goal!`; если Goal!-строк больше текущего total — берутся первые N по времени, типичный VAR); параллельно отдельным сообщением — **GIF гола / отмены** (`GoalEventGifRenderer` + `ApiClient.sendAnimation`, ↑total → «ГОЛ!», ↓total → «ОТМЕНЁН»; флаг `notification.goal-gif.enabled` / `NOTIFICATION_GOAL_GIF_ENABLED`); при входе в live и catch-up на FT — `BettingRecommendationService.freezeAtKickoffIfNeeded`; при переходе матча в `post/ft` — сразу `predictionService.updateByMatch` (очки не зависят только от LISTEN→`sendFullTime`), плюс очистка кэша составов через `ApiClient.evictLineups(publicId)`.
...
- **Live goal GIF:** `GoalEventGifRenderer` (AWT, 400×400, ~3.5с) — фон `notification-cards/goal-anim-bg.png`, логотипы через `TeamLogoCacheService` + ESPN fallback; `GifSequenceWriter` (полные кадры) + **`gifsicle -O0`** для сжатия (ищет `gifsicle` / `/usr/bin` / `~/bin/gifsicle`). На Odyssey: бинарь в `~/bin/gifsicle`, в `deploy/predicts.env` — `PATH=.../home/nikita/bin:...`. Отправка `bot.urlAnimation` / `ApiClient.sendAnimation`. Превью: `./mvnw -Dtest=GoalEventGifPreviewTest test` → `target/preview-cards/goal-*.gif`. Выкл.: `NOTIFICATION_GOAL_GIF_ENABLED=false`.
